...
# Input ts column: ts
# - If --include_time_to_mod is set, we always send input time under fixed key "ts" (epoch seconds, int).
...
def model():
    ...
    for k, l in payload.items():
        ...
        else:  # If it is ts just add 1 (sec, nsec, ...)
            for v in l:
                out_dic[k + '_pred'].append(v + 1)
